...
//Load varieties
    ...
    if (count($params) == 1) {
        ...
        $color = mysqli_fetch_assoc(mysqli_query($mysqli, "select Color, count(*) as magnitude from RTsWQuality WHERE `Grower` = '" . $params[1] . "' and `Var Desc` = '".$params[0]."' and `Farm` = '".$params[2]."' and `Block` = '".$params[3]."' and `BuOnHand` != 0 and Color != '' Group By Color order by magnitude desc limit 1"));
        $query = mysqli_query($mysqli, "SELECT `RT#`, round(avg(PressureAvg),2) as Pressure, round(avg(CASE WHEN `DAAvg` <> '' THEN DAAvg ELSE NULL END),2) as DA, round(avg(CASE WHEN `Brix` <> '' THEN Brix ELSE NULL END),2) as Brix, round(avg(`AverageWeight`),2) as Weight, format(sum(`BuOnHand`),0) as OH FROM RTsWQuality WHERE `Grower` = '" . $params[1] . "' and `Var Desc` = '".$params[0]."' and `Farm` = '".$params[2]."' and `Block` = '".$params[3]."'  and `BuOnHand` != 0 and isQA != 'FALSE' GROUP BY `Block`");
