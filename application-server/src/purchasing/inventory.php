        ...
        $.getJSON('API/getItemTypes.php', function (data) {
            for (var key in data) {
                if (data.hasOwnProperty(key)) {
                    $('#fillMeWithTypes').append("<button onclick=\"curTypeName='" + data[key]['Type_Description'] + "',loadProductsForType(" + data[key]['Type_ID'] + ", '" + data[key]['Type_Description'] + "' ,true)\" class=\"mdl-button mdl-js-button mdl-button--primary mdl-js-ripple-effect mdl-shadow--6dp mdl-cell mdl-cell--4-col mdl-cell--3-col-desktop\" style=\"height: 125px; float: left; margin: 6px; border-radius: 12px; word-wrap: nowrap; text-overflow:ellipsis; text-align: center; font-size: x-large; vertical-align: middle\">" + data[key]['Type_Description'] + "<p style='font-size: small;position: absolute; width: 100%; left: 0; color: #757575'>(" + data[key]['ItemCount'] + " items)</p></button></div>");
