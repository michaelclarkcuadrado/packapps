...
if (isset($_FILES[0]) && isset($_POST['ID'])) {
    if (strpos($_POST['ID'], 'confirmation_') !== false) {
        if (!file_exists('../assets/Assets/Order_Confirmations/'. str_replace('confirmation_', '', $_POST['ID']) . '_confirm.pdf') && strpos($_FILES[0]['name'], '.pdf') !== false) {
            move_uploaded_file($_FILES[0]['tmp_name'], '../assets/Assets/Order_Confirmations/' . str_replace('confirmation_', '', $_POST['ID']) . '_confirm.pdf');
