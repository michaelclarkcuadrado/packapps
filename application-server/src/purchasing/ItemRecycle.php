...
if (!isset($_COOKIE['auth']) || !isset($_COOKIE['username'])) {
    ...
    $checkAllowed = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT allowedPurchasing, `Real Name` as RealName, Role AS Role FROM packapps_master_users JOIN purchasing_UserData ON packapps_master_users.username=purchasing_UserData.Username WHERE packapps_master_users.username = '$SecuredUserName'"));
            ...
            for (var item in data) {
                if (data.hasOwnProperty(item)) {
                        string += "<tr><td style='font-size: 25px; white-space: normal; line-height: initial' class='mdl-data-table__cell--non-numeric'>"
                            + data[item]['ItemDesc']
                            + "</td><td><i style='cursor:pointer' class='material-icons' href='#' onclick='enableItem("
                            + data[item]['Item_ID']
