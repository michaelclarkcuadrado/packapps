...
if ($_POST['RunID'] != 'DEFAULT') {
    mysqli_query($mysqli, "DELETE FROM production_runs WHERE RunID='" . $_POST['RunID'] . "'") or APIFail("Could not delete run.");
    $ID = mysqli_real_escape_string($mysqli, $_POST['RunID']);
} else {
    $ID = mysqli_real_escape_string($mysqli, $_POST['RunID']);
}

$insert = mysqli_query($mysqli, "INSERT INTO production_runs VALUES (" . $ID . ", '" . mysqli_real_escape_string($mysqli, $_POST['runNum']) . "', '" . $_POST['Line'] . "', 0, " . $_POST['isPreInspected'] . ", " . $_POST['isQA'] . ", DEFAULT, '" . mysqli_real_escape_string($mysqli, $RealName['UserRealName']) . "')") or APIFail("Could not insert run.");
...
for ($i = 1; $_POST['productMade' . $i]; $i++) {
    mysqli_query($mysqli, "INSERT INTO production_product_needed VALUES ('" . $ID . "', '" . mysqli_real_escape_string($mysqli, $_POST['productMade' . $i]) . "', '" . mysqli_real_escape_string($mysqli, $_POST['packSize' . $i]) . "', '" . mysqli_real_escape_string($mysqli, $_POST['madeAmount' . $i]) . "', '" . mysqli_real_escape_string($mysqli, $_POST['amountType' . $i]) . "')") or APIFail("Could not add product.");
}
