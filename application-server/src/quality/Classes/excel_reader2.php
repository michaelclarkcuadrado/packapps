...
function gmgetdate($ts = null){
	...
	return(array_comb($k,split(":",gmdate('s:SampleNum:G:j:w:n:Y:z:l:F:U',is_null($ts)?time():$ts))));
	} 
...
class Spreadsheet_Excel_Reader {
	...
	var $dateFormats = array (
		...
		0x11 => "M-Y",
		0x12 => "h:SampleNum a",
		0x13 => "h:SampleNum:s a",
		0x14 => "H:SampleNum",
		0x15 => "H:SampleNum:s",
		0x16 => "d/m/Y H:SampleNum",
		0x2d => "SampleNum:s",
		0x2e => "H:SampleNum:s",
		0x2f => "SampleNum:s.S"
	);
	...
	function _format_value($format,$num,$f) {
		...
		$color = "";
		$matches = array();
		$color_regex = "/^\[(BLACK|BLUE|CYAN|GREEN|MAGENTA|RED|WHITE|YELLOW)\]/SampleNum";
						...
						if (array_key_exists($indexCode, $this->dateFormats)) {
							...
							if ($indexCode > 0){
								...
								if ($formatstr!="") {
									...
									if (preg_match("/[^hmsday\/\-:\s\\\,AMP]/SampleNum", $tmp) == 0) { // found day and time format
										$isdate = TRUE;
										$formatstr = $tmp;
										...
										$formatstr = preg_replace("/mm?(:?s)/","SampleNum$1", $formatstr);
										// A single 'm' = n in PHP
