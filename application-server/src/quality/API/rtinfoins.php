...
$alreadyDoneSamples = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT `#Samples` FROM quality_InspectedRTs WHERE `RTNum` = '$RT'"))['#Samples'];
if($alreadyDoneSamples == null){
    $query2 = mysqli_query($mysqli, "SELECT (CASE WHEN (count(`RTNum`))>=1 THEN 5 ELSE 10 END) AS NumSamplesRequired FROM quality_InspectedRTs JOIN BULKOHCSV ON `RT#`=RTNum WHERE GrowerName='" . $output['GrowerName'] . "' AND VarDesc='" . $output['VarDesc'] . "' AND StrDesc='" . $output['StrDesc'] . "' AND FarmDesc='" . $output['FarmDesc'] . "' AND BlockDesc='" . $output['BlockDesc'] . "' AND DATE(`DateInspected`)=curdate()");
...
//perform additional test to see if it is first in season
    if ($output2['NumSamplesRequired'] == 10 && ($output['CommDesc'] != 'Peach' && $output['CommDesc'] != 'Nectarine')) {
        $query2 = mysqli_query($mysqli, "SELECT (CASE WHEN (count(`RTNum`))>=1 THEN 10 ELSE 20 END) AS NumSamplesRequired FROM quality_InspectedRTs JOIN BULKOHCSV ON `RT#`=RTNum WHERE GrowerName='" . $output['GrowerName'] . "' AND VarDesc='" . $output['VarDesc'] . "' AND StrDesc='" . $output['StrDesc'] . "' AND FarmDesc='" . $output['FarmDesc'] . "' AND BlockDesc='" . $output['BlockDesc'] . "' ");
        $output2 = mysqli_fetch_assoc($query2);
    }
