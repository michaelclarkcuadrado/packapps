...
$blockdata = mysqli_query($mysqli, "SELECT `Comm Desc`,VarDesc,FarmDesc,BlockDesc,`Str Desc`,format(".(date('Y')-5)."act,0),format(".(date('Y')-4)."act,0),format(".(date('Y')-3)."act,0),format(".(date('Y')-2)."act,0),format(".(date('Y')-1)."act,0),format(".date('Y')."est,0) FROM `grower_crop-estimates` WHERE Grower='" . ($admin[0] == 1 && ($_GET['pretend']) ? $_GET['pretend'] : (isset($_GET['alt_acc']) ? base64_decode($_GET['alt_acc']) : $_SERVER['PHP_AUTH_USER'])) . "' AND isDeleted = '0' ORDER BY `Comm Desc`, VarDesc, FarmDesc, BlockDesc, `Str Desc` ASC;");
if (!$blockdata) {
