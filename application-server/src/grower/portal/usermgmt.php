...
//security check
$namecnct = mysqli_query($mysqli, "SELECT isAdmin FROM `grower_growerLogins` WHERE GrowerCode='" . $_SERVER['PHP_AUTH_USER'] . "'");
...
if ($_GET['del']) {
    ...
    mysqli_query($mysqli, "DELETE FROM grower_growerLogins WHERE GrowerCode='" . mysqli_real_escape_string($mysqli, $_GET['del']) . "'") or die(mysqli_error($mysqli));
...
if ($_POST['GrowerName']) {
    ...
    mysqli_query($mysqli, "INSERT INTO grower_growerLogins (GrowerCode, GrowerName, Password, isAdmin) VALUES ('" . mysqli_real_escape_string($mysqli, $_POST['GrowerCode']) . "', '" . mysqli_real_escape_string($mysqli, $_POST['GrowerName']) . "', '" . mysqli_real_escape_string($mysqli, $_POST['grwrpasswd']) . "', '" . ($_POST['newisadmin'] == 'Yes' ? '1' : '0') . "'); ") or die("Could not add grower! " . mysqli_error($mysqli));
...
//change password
if ($_POST['password']) {
    mysqli_query($mysqli, "UPDATE grower_growerLogins SET `Password`='" . mysqli_real_escape_string($mysqli, $_POST['password']) . "' WHERE GrowerCode='" . mysqli_real_escape_string($mysqli, $_POST['username']) . "'");
...
//print for user
$userlist = mysqli_query($mysqli, "SELECT GrowerName, GrowerCode, ifnull(Password, 'Unknown') as Password, isAdmin FROM grower_growerLogins ORDER BY isAdmin DESC, GrowerCode");
?>
...
<table style="margin-left: auto; margin-right: auto; border: solid black 1px">
    ...
    <? while ($userarray = mysqli_fetch_assoc($userlist)) {
        echo "<tr><td><b>" . $userarray['GrowerName'] . "</b></td><td><form action='usermgmt.php' method='post'><input type='text' name='username' value='" . $userarray['GrowerCode'] . "' readonly></td><td>" . ($userarray['isAdmin'] == '1' ? '<mark>Staff</mark>' : 'Grower') . "</td><td>" . ($userarray['isAdmin'] == '1' ? '' : '<a href=\'/portal?pretend=' . $userarray['GrowerCode'] . '\'>Log In</a>') . "</td><td>Current Password: <b>" . ($userarray['isAdmin'] == 1 ? '' : $userarray['Password']) . "</b><br><br>Change Password to: <input type='password' name='password' value=''><br><input type='submit'></form><br></td><td><a href='usermgmt.php?del=" . $userarray['GrowerCode'] . "'>Delete</a></td></tr>";
    } ?>
</table>
<script>
    function logout() {
