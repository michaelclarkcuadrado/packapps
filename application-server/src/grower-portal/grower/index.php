...
<head>
    ...
    $adminauth = mysqli_query($mysqli, "SELECT isAdmin, isMultiAccountUser FROM GrowerData WHERE GrowerCode='" . $_SERVER['PHP_AUTH_USER'] . "'");
    ...
    if ($admin['isMultiAccountUser'] == 1) {
        $groupID = mysqli_fetch_array(mysqli_query($mysqli, "SELECT GroupID from GrowerGroups where GrowerCode='" . $_SERVER['PHP_AUTH_USER'] . "'"))[0];
        if (isset($_GET['alt_acc'])) {
            $checkIfOnList = mysqli_query($mysqli, "SELECT GrowerCode FROM GrowerGroups where GroupID = '" . $groupID . "'");
            $isLegit = false;
    ...
    $blockCompletionData = mysqli_query($mysqli, "select `growerReporting`.`crop-estimates`.`PK` AS `PK`,
  ...
  from (`growerReporting`.`crop-estimates` 
    join `growerReporting`.`BULKRTCSV` 
  ...
  group by `growerReporting`.`crop-estimates`.`PK`
    ...
    $numPreHarvest = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT COUNT(*) as count FROM (SELECT PK FROM `Preharvest_Samples` WHERE Grower= '" . ($admin[0] == 1 && isset($_GET['pretend']) ? $_GET['pretend'] : (isset($_GET['alt_acc']) ? base64_decode($_GET['alt_acc']) : $_SERVER['PHP_AUTH_USER'])) . "' AND `Date` >= (NOW() - INTERVAL 7 DAY) GROUP BY `Date`, `PK`) t1"))['count'];
    ...
    <noscript><p><img src="//grower.ricefruit.com/analytics/piwik.php?idsite=1" style="border:0;" alt=""/></p>
    </noscript>
    <!-- End Piwik Code -->
</head>
<script>
...
<div id="header" class="skel-layers-fixed">
    ...
    <div class="top">
        ...
        <div id="logo">
            ...
            <p><? echo $companyName ?> Grower<br></p>
        </div>
...
<div id="main">
    ...
    if ($admin[0] == 1 && $_GET['pretend']) {
        ...
        echo "<div style='position: fixed; top: 0; text-align: center; margin 0 auto; display: block; background-color: black; color: red; opacity: .78; padding: 3px'>Viewing as: \"" . (isset($_GET['alt_acc']) ? base64_decode($_GET['alt_acc']) : $_SERVER['PHP_AUTH_USER']) . "\"<br><small>Switch to: </small><select onchange=\"window.location = '/grower/?alt_acc='+$(this).val() \"><option disabled selected>" . (isset($_GET['alt_acc']) ? base64_decode($_GET['alt_acc']) : $_SERVER['PHP_AUTH_USER']) . "</option>";
        $growerAuthedList = mysqli_query($mysqli, "SELECT GrowerCode FROM GrowerGroups where GroupID = '" . $groupID . "'");
        ...
        <div class="container">
            ...
            <header>
                <h2 class="alt"><strong><?php echo $growername ?></strong> Grower Control Panel<br/>
                    at <strong><? echo $companyName ?> Company</strong></h2>
